...
/**
 * Delete a workshop permanently from the database
 * @param workshopId Workshop ID to delete
 * @returns void
 */
export const deleteWorkshop = async (workshopId: string): Promise<void> => {
  try {
    // First delete any registrations for this workshop
    ...
    // Then delete the workshop itself
    const { error } = await supabase
      .from('workshops')
      .delete()
      .eq('id', workshopId);
 ...
 * @returns void
 */
export const deleteServiceFromDB = async (serviceId: string): Promise<void> => {
  try {
    // First, delete any service bookings
    ...
    if (messagesError) {
      console.error('Error deleting service messages:', messagesError);
    }
    ...
    console.error('Error in deleteServiceFromDB:', error);
    throw new Error(error.message || 'Failed to delete service');
  }
};

/**
 * Delete a group permanently from the database
 * @param groupId Group ID to delete
 * @returns void
 */
export const deleteGroupFromDB = async (groupId: string): Promise<void> => {
  try {
    ...
    console.error('Error in deleteGroupFromDB:', error);
    throw new Error(error.message || 'Failed to delete group');
  }
};

/**
 * Delete an event permanently from the database
 * @param eventId Event ID to delete
 * @returns void
 */
export const deleteEventFromDB = async (eventId: string): Promise<void> => {
  try {
    // First delete all join requests for this event
    const { error: requestsError } = await supabase
      .from('join_requests')
      .delete()
      .eq('event_id', eventId);
    ...
    if (requestsError) {
      console.error('Error deleting event join requests:', requestsError);
    }
...
/**
 * Delete a job posting permanently from the database
...
/**
 * Delete a sponsorship permanently from the database
 * @param sponsorshipId Sponsorship ID to delete
 * @returns void
 */
export const deleteSponsorshipFromDB = async (sponsorshipId: string): Promise<void> => {
  try {
    // First delete all applications for this sponsorship
    ...
    if (applicationsError) {
      console.error('Error deleting sponsorship applications:', applicationsError);
    }
    
    // Then delete the sponsorship itself
    const { error } = await supabase
      .from('sponsorships')
      .delete()
      .eq('id', sponsorshipId);
    ...
    console.error('Error in deleteSponsorshipFromDB:', error);
    throw new Error(error.message || 'Failed to delete sponsorship');
  }
};
