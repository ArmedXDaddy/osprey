...
const PostItem: React.FC<PostItemProps> = ({ post }) => {
  ...
  return (
    <Card className="overflow-hidden mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              <span className={`inline-block px-2 py-0.5 text-xs rounded-full capitalize
                ${post.userRole === 'influencer' ? 'bg-purple-100 text-purple-800' : 
                  post.userRole === 'coach' ? 'bg-blue-100 text-blue-800' : 
                  post.userRole === 'company' ? 'bg-green-100 text-green-800' : 
      ...
      <CardFooter className="py-3 px-4 flex justify-between border-t">
        <Button 
          variant="ghost" 
          size="sm" 
          onClick={handleLike}
